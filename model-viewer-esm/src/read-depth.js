import { FILTER_NEAREST, PIXELFORMAT_RGBA8, SEMANTIC_POSITION, drawQuadWithShader, shaderChunks, BlendState, RenderTarget, Texture, createShaderFromCode } from 'playcanvas';
...
class ReadDepth {
    device = undefined;
    shader = undefined;
    depthTexUniform = undefined;
    texcoordRangeUniform = undefined;
    ...
    /**
     * @param {Texture} depthTexture
     * @param {number} x
     * @param {number} y
     * @returns {number}
     */
    read(depthTexture, x, y) {
        if (!this.texture) {
            this.create();
        }
